function truthCheck(collection, pre) {
  ...
  var beTrue = true;  // Boolean value to check.
  
  for( var i = 0; i < collection.length; i++){  // Loop the array.
    
    if (!collection[i][pre]){  // If there is no pre value in a collection element, set the boolean to false.
      
      beTrue = false;
    }
  }
  
  return beTrue;  // Return boolean.
}
